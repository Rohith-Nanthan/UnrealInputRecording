...
namespace RecordingBootFlags
{
	 ...
	 */
	UNREALINPUTRECORDING_API bool FindSwitchValue(const TCHAR* CommandLine, const TCHAR* Key, FString& OutValue, bool& bOutHadValue);

	/** Convenience wrapper for callers that do not care whether a value was attached. */
	UNREALINPUTRECORDING_API bool HasSwitch(const TCHAR* CommandLine, const TCHAR* Key);

	/** Parses without applying anything. Exposed so tests and logging can reuse it. */
	UNREALINPUTRECORDING_API FRecordingBootFlags Parse(const TCHAR* CommandLine);

	/** The flags this process booted with. */
	UNREALINPUTRECORDING_API const FRecordingBootFlags& Get();

	/** Called once from the game module's StartupModule. */
	UNREALINPUTRECORDING_API void InitializeFromCommandLine();

	/** Human-readable one-liner for logs and menus. */
	UNREALINPUTRECORDING_API FString Describe();
}
